...
메인 페이지 오른쪽 영역의 항목 순서는 `settings.json`의 `main_page_sections` 배열에서 관리합니다. 배열 안에서 다음 키의 위치를 옮기면 화면에도 같은 순서로 반영됩니다.
...
지원되는 키를 각각 정확히 한 번씩 입력해야 합니다. 항목 누락, 중복 또는 정의되지 않은 키가 있으면 데이터 검증과 페이지 로딩이 실패합니다.
