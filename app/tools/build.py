...
def validate_references(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        for reference_index, reference in enumerate(references, start=1):
            ...
            elif reference not in valid_ids:
...
def validate(data: dict[str, Any]) -> list[str]:
    ...
    hidden_main_page_sections = settings.get("hidden_main_page_sections", [])
    configured_main_page_sections = (
        main_page_sections + hidden_main_page_sections
        if isinstance(main_page_sections, list)
        and isinstance(hidden_main_page_sections, list)
        else []
    )
    if (
        not isinstance(main_page_sections, list)
        or not isinstance(hidden_main_page_sections, list)
        or not all(isinstance(section, str) for section in configured_main_page_sections)
        or len(configured_main_page_sections) != len(MAIN_PAGE_SECTIONS)
        or len(set(configured_main_page_sections)) != len(MAIN_PAGE_SECTIONS)
        or set(configured_main_page_sections) != set(MAIN_PAGE_SECTIONS)
    ):
        errors.append(
            "settings.json: main_page_sections and hidden_main_page_sections must together "
            "contain each supported section exactly once: "
