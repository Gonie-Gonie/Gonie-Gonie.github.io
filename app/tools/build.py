...
CANONICAL_ID_PATTERN = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)*$")
MAIN_PAGE_SECTIONS = [
    "experience",
    "education",
    "scholarships",
...
LEGACY_MAIN_PAGE_SECTIONS = [
    section for section in MAIN_PAGE_SECTIONS if section != "academic_activities"
]
CV_SECTIONS = [
    "experience",
    "education",
...
ACADEMIC_ACTIVITY_FIELDS = {
    "reviews": ["journal_en", "journal_ko", "completed_dates"],
    "invited_talks": [
        "event_en", "event_ko", "topic_en", "topic_ko", "date"
    ],
    "conference_service": [
...
def is_canonical_id(value: Any) -> bool:
    """Return whether value is an exact, canonical JSON string ID."""
...
def unique_ids(rows: list[dict[str, Any]], label: str, errors: list[str]) -> set[str]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if not is_canonical_id(value):
            errors.append(
                f"{label} item {index}: id must be a JSON string matching "
                "^[a-z0-9]+(?:-[a-z0-9]+)*$ exactly."
            )
            continue
        ids.append(value)
...
def validate_fields(
    ...
    for index, row in enumerate(value, start=1):
        ...
        unexpected = sorted(actual_fields - expected_fields - optional_fields)
        if missing:
...
def validate_cv_visibility(item: Any, context: str, errors: list[str]) -> None:
    """Validate the optional per-item CV flag (missing means visible)."""
    if (
        isinstance(item, dict)
        and "visible_in_CV" in item
        and not isinstance(item["visible_in_CV"], bool)
    ):
        errors.append(f"{context}: visible_in_CV must be a boolean when present.")


def validate_section_settings(
    ...
    *,
    ...
    visible = settings.get(visible_key)
    hidden = settings.get(hidden_key)
    if allow_legacy_missing and visible is None and hidden is None:
        return
    configured = visible + hidden if isinstance(visible, list) and isinstance(hidden, list) else []
    if (
        not isinstance(visible, list)
        or not isinstance(hidden, list)
        or not all(isinstance(section, str) for section in configured)
        or len(configured) != len(supported)
        or len(set(configured)) != len(supported)
        or set(configured) != set(supported)
    ):
        errors.append(
            f"settings.json: {visible_key} and {hidden_key} must together contain "
            "each supported section exactly once: " + ", ".join(supported) + "."
        )


def normalize_main_page_section_settings(settings: dict[str, Any]) -> None:
    """Preserve v5 layouts by adding the new section to the hidden list in memory."""
    visible = settings.get("main_page_sections")
    hidden = settings.get("hidden_main_page_sections")
    configured = visible + hidden if isinstance(visible, list) and isinstance(hidden, list) else []
    ...
    if (
        ...
        and len(configured) == len(LEGACY_MAIN_PAGE_SECTIONS)
        and len(set(configured)) == len(LEGACY_MAIN_PAGE_SECTIONS)
        and set(configured) == set(LEGACY_MAIN_PAGE_SECTIONS)
    ):
        settings["hidden_main_page_sections"] = [*hidden, "academic_activities"]


def validate_academic_activities(value: Any, errors: list[str]) -> None:
    context = "academic_activities.json"
    if not isinstance(value, dict):
        errors.append(f"{context}: top-level value must be an object.")
        return

    expected_top_level = {"schema_version", *ACADEMIC_ACTIVITY_FIELDS}
    missing_top_level = sorted(expected_top_level - set(value))
    if missing_top_level:
        errors.append(f"{context}: missing field(s): {', '.join(missing_top_level)}.")
    schema_version = value.get("schema_version")
    if not isinstance(schema_version, int) or isinstance(schema_version, bool) or schema_version != 1:
        errors.append(f"{context}: schema_version must be 1.")

    for category, fields in ACADEMIC_ACTIVITY_FIELDS.items():
        rows = value.get(category)
        if not isinstance(rows, list):
            errors.append(f"{context} {category}: top-level value must be an array.")
            continue
        for index, item in enumerate(rows, start=1):
            item_context = f"{context} {category} item {index}"
            if not isinstance(item, dict):
                errors.append(f"{item_context}: must be an object.")
                continue
            validate_cv_visibility(item, item_context, errors)
            missing_fields = [field for field in fields if field not in item]
            if missing_fields:
                errors.append(
                    f"{item_context}: missing field(s): {', '.join(missing_fields)}."
                )
            for field in fields:
                if field != "completed_dates" and not isinstance(item.get(field), str):
                    errors.append(f"{item_context}: {field} must be a string.")

            if category == "reviews":
                completed_dates = item.get("completed_dates")
                if not isinstance(completed_dates, list):
                    errors.append(f"{item_context}: completed_dates must be an array.")
                else:
                    if not completed_dates:
                        errors.append(
                            f"{item_context}: completed_dates must contain at least one date."
                        )
                    seen_dates: set[str] = set()
                    for date_index, date_value in enumerate(completed_dates):
                        date_context = f"{item_context}: completed_dates[{date_index}]"
                        valid_date = (
                        ...
                        if not valid_date:
                            errors.append(
                                f"{date_context} must be a valid YYYY-MM-DD date."
                            )
                        if isinstance(date_value, str):
                            if date_value in seen_dates:
                                errors.append(
                                    f"{item_context}: completed_dates must not contain "
                                    f"duplicate date '{date_value}'."
                                )
                            seen_dates.add(date_value)
                localized_pairs = [("journal_en", "journal_ko")]
            elif category == "invited_talks":
                localized_pairs = [("event_en", "event_ko"), ("topic_en", "topic_ko")]
            elif category == "conference_service":
                localized_pairs = [
                    ("conference_en", "conference_ko"), ("role_en", "role_ko")
                ]
            elif category == "professional_service":
                localized_pairs = [
                    ("organization_en", "organization_ko"), ("role_en", "role_ko")
                ]
            else:
                localized_pairs = [("journal_en", "journal_ko"), ("role_en", "role_ko")]

            for localized_pair in localized_pairs:
                if not any(
                    isinstance(item.get(field), str) and item[field].strip()
                    for field in localized_pair
                ):
                    errors.append(
                        f"{item_context}: at least one localized value is required for "
                        f"{'/'.join(localized_pair)}."
                    )

            if category == "invited_talks":
                date_value = item.get("date")
                if not isinstance(date_value, str) or not re.fullmatch(r"\d{4}-\d{2}-\d{2}", date_value):
                    errors.append(f"{item_context}: date must be a valid YYYY-MM-DD date.")
                else:
                    try:
                        datetime.strptime(date_value, "%Y-%m-%d")
                    except ValueError:
                ...
                "conference_service", "professional_service", "editorial_service"
            }:
                parsed_dates: dict[str, datetime] = {}
                for field in ("start_date", "end_date"):
                    date_value = item.get(field)
                    if field == "end_date" and date_value == "":
                        continue
                    if not isinstance(date_value, str) or not re.fullmatch(
                        r"\d{4}-\d{2}-\d{2}", date_value
                    ):
                        errors.append(
                            f"{item_context}: {field} must be "
                            + ("blank or " if field == "end_date" else "")
                            + "a valid YYYY-MM-DD date."
                        )
                        continue
                    try:
                        parsed_dates[field] = datetime.strptime(date_value, "%Y-%m-%d")
                    except ValueError:
                        errors.append(
                            f"{item_context}: {field} must be "
                            + ("blank or " if field == "end_date" else "")
                ...
                if (
                    ...
                    errors.append(f"{item_context}: start_date must not be later than end_date.")


def validate_profile_scholarships(value: Any, errors: list[str]) -> None:
    context = "profile.json scholarships"
    if not isinstance(value, list):
        errors.append(f"{context} must be a list.")
        return
    for index, scholarship in enumerate(value, start=1):
        item_context = f"profile.json: scholarships[{index}]"
        if not isinstance(scholarship, dict):
            errors.append(f"{item_context} must be an object.")
            continue
        validate_cv_visibility(scholarship, item_context, errors)
        if "details" in scholarship:
            errors.append(f"{item_context}.details is obsolete; use amount instead.")
        amount = scholarship.get("amount")
        if not isinstance(amount, dict):
            errors.append(f"{item_context}.amount must be an object.")
            continue
        amount_value = amount.get("value")
        if (
            not isinstance(amount_value, (int, float))
            or isinstance(amount_value, bool)
            or (isinstance(amount_value, float) and not math.isfinite(amount_value))
            or amount_value <= 0
        ):
            errors.append(
                f"{item_context}.amount.value must be a positive finite number."
            )
        currency = amount.get("currency")
        if not isinstance(currency, str) or not re.fullmatch(r"[A-Z]{3}", currency):
            errors.append(
                f"{item_context}.amount.currency must be a three-letter uppercase code."
            )
...
def validate_references(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if not isinstance(references, list) or not references:
            errors.append(
                f"{label} '{item_label}': {field} must be a non-empty array "
                "of canonical ID strings."
            )
            continue
        seen_references: set[str] = set()
        for reference_index, reference in enumerate(references, start=1):
            if not is_canonical_id(reference):
                errors.append(
                    f"{label} '{item_label}': {field} reference at position "
                    f"{reference_index} must be a JSON string matching "
                    "^[a-z0-9]+(?:-[a-z0-9]+)*$ exactly."
                )
                continue
            if reference in seen_references:
                errors.append(
                    f"{label} '{item_label}': duplicate {field} reference "
                    f"'{reference}' at position {reference_index}."
                )
            else:
...
def validate(data: dict[str, Any]) -> list[str]:
    ...
    publications = validate_fields(
        ...
        "publications.json",
        errors,
        optional=("visible_in_CV",),
    )
    awards = validate_fields(
        data["awards"],
        AWARD_FIELDS,
        "awards.json",
        errors,
    ...
    board = data["board"]
    academic_activities = data["academic_activities"]

    normalize_main_page_section_settings(settings)
    ...
    validate_section_settings(
        ...
        allow_legacy_missing=(
            isinstance(settings.get("schema_version"), int)
            and not isinstance(settings.get("schema_version"), bool)
            and settings["schema_version"] <= 4
        ),
    )
    ...
    validate_academic_activities(academic_activities, errors)

    for index, award in enumerate(awards, start=1):
        context = f"awards.json item {index}"
        validate_cv_visibility(award, context, errors)
        ...
        if isinstance(award_date, str):
            if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", award_date):
                errors.append(f"{context}: date must use YYYY-MM-DD exactly.")
            else:
                try:
                    datetime.strptime(award_date, "%Y-%m-%d")
                except ValueError:
                    errors.append(f"{context}: invalid date '{award_date}'.")
        for localized_field, label in (
            (("title_en", "title_ko"), "title"),
            (("organization_en", "organization_ko"), "organization"),
        ):
            if not any(
                isinstance(award.get(field), str) and award[field].strip()
                for field in localized_field
            ):
                errors.append(
                    f"{context}: at least one localized {label} is required."
                )

    for index, item in enumerate(software, start=1):
        context = f"software.json item {index}"
        validate_cv_visibility(item, context, errors)
    ...
    if not isinstance(project_themes, list):
        errors.append("settings.json: project_themes must be a list.")
        project_themes = []
    project_theme_ids = unique_ids(project_themes, "settings.json project_themes", errors)
    for theme in project_themes:
        theme_id = theme.get("id", "<missing>")
        for language in ("en", "ko"):
            label = theme.get(f"label_{language}")
            if not isinstance(label, str) or not label.strip():
                errors.append(
                    f"settings.json project theme '{theme_id}': "
                    f"label_{language} is required."
                )
    for language in ("en", "ko"):
        labels = [
            str(theme.get(f"label_{language}") or "").strip().casefold()
            for theme in project_themes
        ]
        duplicates = [
        ...
        if duplicates:
            errors.append(
                f"settings.json project_themes: duplicate label_{language} value(s)."
            )

    project_theme_fallback = settings.get("project_theme_fallback", {})
    if not isinstance(project_theme_fallback, dict):
        errors.append("settings.json: project_theme_fallback must be an object.")
    else:
        for language in ("en", "ko"):
            label = project_theme_fallback.get(f"label_{language}")
            if not isinstance(label, str) or not label.strip():
                errors.append(
                    f"settings.json: project_theme_fallback.label_{language} is required."
                )

    for index, project in enumerate(projects, start=1):
        context = f"projects.json item {index}"
        validate_cv_visibility(project, context, errors)
        if "id" in project:
        ...
        if not is_canonical_id(project_theme):
            errors.append(
                f"{context}: theme must be a JSON string matching "
                "^[a-z0-9]+(?:-[a-z0-9]+)*$ exactly."
            )
        elif project_theme not in project_theme_ids:
            errors.append(f"{context}: unknown theme reference '{project_theme}'.")

    publication_topics = settings.get("publication_topics", [])
    if not isinstance(publication_topics, list):
        errors.append("settings.json: publication_topics must be a list.")
        publication_topics = []
    publication_topic_ids = unique_ids(
        publication_topics, "settings.json publication_topics", errors
    )
    for topic in publication_topics:
        topic_id = topic.get("id", "<missing>")
        for language in ("en", "ko"):
            label = topic.get(f"label_{language}")
            if not isinstance(label, str) or not label.strip():
                errors.append(
                    f"settings.json publication topic '{topic_id}': "
                    f"label_{language} is required."
                )
    for language in ("en", "ko"):
        labels = [
            str(topic.get(f"label_{language}") or "").strip().casefold()
            for topic in publication_topics
        ]
        duplicates = [
            label for label, count in Counter(labels).items() if label and count > 1
        ...
        if duplicates:
            errors.append(
                f"settings.json publication_topics: duplicate label_{language} value(s)."
            )

    topic_fallback = settings.get("publication_topic_fallback", {})
    if not isinstance(topic_fallback, dict):
        errors.append("settings.json: publication_topic_fallback must be an object.")
    else:
        for language in ("en", "ko"):
            label = topic_fallback.get(f"label_{language}")
            if not isinstance(label, str) or not label.strip():
                errors.append(
                    f"settings.json: publication_topic_fallback.label_{language} is required."
                )

    for index, publication in enumerate(publications, start=1):
        context = f"publications.json item {index}"
        validate_cv_visibility(publication, context, errors)
        publication_topic = publication.get("topic")
        if isinstance(publication_topic, str) and publication_topic:
            if not is_canonical_id(publication_topic):
                errors.append(
                    f"{context}: non-empty topic must match "
                    "^[a-z0-9]+(?:-[a-z0-9]+)*$ exactly."
                )
            elif publication_topic not in publication_topic_ids:
                errors.append(
                    f"{context}: unknown topic reference '{publication_topic}'."
                )
        ...
        if isinstance(raw_award_id, str) and raw_award_id:
            if not is_canonical_id(raw_award_id):
                errors.append(
                    f"{context}: non-empty award_id must match "
                    "^[a-z0-9]+(?:-[a-z0-9]+)*$ exactly."
                )
            elif raw_award_id not in award_ids:
                errors.append(
                    f"{context}: unknown award_id reference '{raw_award_id}'."
                )
    ...
    for section_name in PROFILE_CV_SECTIONS:
        if section_name == "scholarships":
            continue
        section_items = profile.get(section_name)
        if not isinstance(section_items, list):
            errors.append(f"profile.json: {section_name} must be a list.")
            continue
        for index, item in enumerate(section_items, start=1):
            context = f"profile.json {section_name} item {index}"
            if not isinstance(item, dict):
                errors.append(f"{context}: must be an object.")
                continue
            validate_cv_visibility(item, context, errors)
    validate_profile_scholarships(profile.get("scholarships"), errors)
...
def load_all() -> dict[str, Any]:
    return {
        ...
        "academic_activities": read_json("academic_activities.json"),
    }
...
def generate(data: dict[str, Any]) -> None:
    ...
    report = {
        ...
        "counts": {
            ...
            "academic_activities": sum(
                len(data["academic_activities"].get(category, []))
                for category in ACADEMIC_ACTIVITY_FIELDS
            ),
        },
