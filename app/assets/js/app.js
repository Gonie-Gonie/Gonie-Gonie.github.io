...
(function () {
  ...
  function applyMainPageSectionOrder() {
    const order = state.data.settings.main_page_sections;
    ...
    if (!valid) {
      ...
      throw new Error(state.lang === "ko"
        ? `settings.json의 main_page_sections에는 다음 항목이 각각 정확히 한 번씩 있어야 합니다: ${expected}`
        : `settings.json main_page_sections must contain each supported section exactly once: ${expected}`);
    }
    ...
    const container = sectionNodes.get(order[0]).parentElement;
